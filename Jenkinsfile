    ...
    stages {
        ...
        stage('Selenium Grid setup') { 
            steps { 
                ...
                sh "docker run -d --rm -p 4444:4444 --net=SEL --name my-selenium-hub selenium/hub" 
                sh "docker run -d --rm --net=SE -e HUB_HOST=my-selenium-hub --name selenium-node-chrome" 
